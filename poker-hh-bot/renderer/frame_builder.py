"""
Builds a single 1280×720 PIL Image frame representing one poker action.
"""
...
def _paste_card(canvas: Image.Image, card_img: Image.Image, cx: int, cy: int) -> None:
    """Paste a card image centred at (cx, cy)."""
    x = cx - CARD_W // 2
    y = cy - CARD_H // 2
...
def build_frame(
    ...
    """
    Render one frame of the hand video.

    Parameters
    ----------
    hand            : full HandHistory (for player metadata)
    street          : current Street object
    action          : the Action being displayed on this frame
    pot             : running pot total for this frame (dollars)
    folded_positions: set of position strings that have already folded
    action_label    : human-readable string for the ticker bar
    """
    ...
    for pos, (cx, cy) in POSITION_COORDS.items():
        ...
        if player is None and pos not in {p.position for p in hand.players}:
            # Empty seat — draw dimmed placeholder
            ov_draw.rounded_rectangle(
                [bx, by, bx2, by2], radius=6,
                fill=(20, 20, 20, 80), outline=(80, 80, 80, 100), width=1,
            )
            draw_on = ImageDraw.Draw(overlay)
            draw_on.text((bx + 4, by + 4), pos, font=font_sm, fill=(120, 120, 120, 150))
            continue
    ...
    # Centre the text vertically in the bar
    ...
    # Convert to RGB for FFmpeg (drop alpha)
